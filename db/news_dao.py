...
class NewsDao:
    ...
    def search_unreview_list(self, page):
        try:
            ...
            sql = "SELECT n.id, n.title, t.type, u.username " \
                  "FROM t_news n JOIN t_type t ON n.type_id = t.id JOIN t_user u ON n.editor_id = u.id WHERE n.state = %s ORDER BY n.create_time DESC " \
                  "LIMIT %s, %s"
    ...
    def search_all_list(self, page):
        try:
            ...
            cursor = con.cursor()
            sql = "SELECT n.id, n.title, t.type, u.username " \
                  "FROM t_news n JOIN t_type t ON n.type_id = t.id JOIN t_user u ON n.editor_id = u.id ORDER BY n.create_time DESC " \
                  "LIMIT %s, %s"
    ...
    def search_content_id(self, id):
        try:
            con = pool.get_connection()
            cursor = con.cursor()
            sql = "SELECT connect_id FROM t_news WHERE id = %s"
            cursor.execute(sql, [id])
